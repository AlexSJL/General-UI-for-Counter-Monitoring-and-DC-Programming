...
# Generate Command String based on the Communication Protocol of DT8034
def create_command_string(CMD, PAR, CH=None, VAL=None):
    if CMD not in ["MON", "SET", "INFO"]:
        raise ValueError(f'<CMD> must be one of {"MON", "SET", "INFO"}, received {CMD}.')
    command = f'$CMD:{CMD},'
    if CH is not None:
        try:
            CH = int(CH)
        ...
        if not 0 <= CH <= 7:
            raise ValueError(f'<CH> must be one of {{0,1,...,7}}, received {CH}.')
        command += f'CH:{CH},'
    command += f'PAR:{PAR},'
    if VAL is not None:
        command += f'VAL:{VAL},'
...
# Check if response string is string type, otherwise raise an error
...
# Check if a provided variable has the desired type, otherwise raise an error
...
# Check if a provided variable has the desired numeric type, otherwise raise an error        
...
class CAENDesktopHighVoltagePowerSupply:
    ...
    def __init__(self, port=None, ip=None, timeout=1):
        ...
        if ip is not None and port is not None: # This is an error, which connection protocol should we use?
            ...
            print(f'CAEN device connect successfully.')
    ...
    def send_command(self, CMD, PAR, CH=None, VAL=None):
        # Send a command to the CAEN device. The parameters of this method are the ones specified in the user manual.
        bytes2send = create_command_string(CMD=CMD, PAR=PAR, CH=CH, VAL=VAL).encode('ASCII')
    ...
    def query(self, CMD, PAR, CH=None, VAL=None):
        ...
        with self._communication_lock: # Lock it to ensure that the answer I return corresponds to this command. Otherwise another thread could send a new command between my send and my read.
            self.send_command(CMD=CMD, PAR=PAR, CH=CH, VAL=VAL)
            response = self.read_response()
        return response
    
    def get_single_channel_parameter(self, parameter: str, channel: int):
        ...
        response = self.query(CMD='MON', PAR=parameter, CH=channel)
    ...
    def set_single_channel_parameter(self, parameter: str, channel: int, value):
        # Sets the value of some parameter (see "SET commands related to the Channels" in the CAEN user manual.)
        # parameter: This is the <whatever> value in "PAR:whatever" that is specified in the user manual.
        # channel: Integer number specifying the numer of the channel.
        # device: If you have more than 1 device connected in the daisy chain, use this parameter to specify the device number (In the user manual this is the <whatever> that goes in "BD:whatever").
        response = self.query(CMD='SET', PAR=parameter, CH=channel, VAL=value)
        if check_successful_response(response) == False:
            raise RuntimeError(f'Error trying to set the parameter {parameter}. The response from the instrument is: "{response}"')
    
    def ramp_voltage(self, voltage: float, channel: int, ramp_speed_VperSec: float = 5, timeout: float = 10):
    ...
    def channel_status(self, channel: int):
        ...
        status_byte = int(self.query(CMD='MON', PAR='STATUS',CH=channel)[-5:])
...
class OneCAENChannel:
    def __init__(self, caen, channel_number):
        ...
        its usage and avoid confusions with channel numbers."""
